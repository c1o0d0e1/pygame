...
# 初始化音效模組（避免部分平台無法播放音效）
pygame.mixer.init()
###################### 載入音效 ######################
# 載入跳躍音效與彈簧音效
sound_jump = pygame.mixer.Sound(os.path.join(sys.path[0], "jump.mp3"))  # 跳躍音效
sound_spring = pygame.mixer.Sound(os.path.join(sys.path[0], "spring.mp3"))  # 彈簧音效
...
bg_y = 600  # 視窗高度
bg_size = (bg_x, bg_y)
pygame.display.set_caption("Doodle Jump - 步驟12")  # 設定視窗標題
...
class Player:
    def __init__(self, x, y, width, height, color):
        ...
        # --- 步驟12: 載入精靈圖片 ---
        self.sprites = None
        self.sprite_state = "player_right_jumping"  # 預設狀態
        # self.load_sprites()  # 載入精靈圖片 (移除，改由外部呼叫)

    def load_sprites(self, sprites):
        """
        載入玩家精靈圖片，存入 self.sprites 字典
        """
        self.sprites = sprites if sprites else None

    def draw(self, display_area):
        """
        在指定畫布上繪製主角，優先使用精靈圖片，若無則用方塊
        ...
        if self.sprites:
            ...
            if self.velocity_y < 0:
                # 跳躍中
                if self.speed < 0 or pygame.key.get_pressed()[pygame.K_LEFT]:
                    self.sprite_state = "player_left_jumping"
                else:
                    self.sprite_state = "player_right_jumping"
            else:
                # 下落中
                if self.speed < 0 or pygame.key.get_pressed()[pygame.K_LEFT]:
                    self.sprite_state = "player_left_falling"
                else:
                    self.sprite_state = "player_right_falling"
            sprite_img = self.sprites.get(self.sprite_state, None)
        if sprite_img:
            # 以精靈圖片繪製，並縮放至主角大小
            sprite_img = pygame.transform.smoothscale(
                sprite_img, (self.width, self.height)
            )
            display_area.blit(sprite_img, (self.rect.x, self.rect.y))
        else:
            # 若無精靈圖片則以方塊繪製
    ...
    def draw(self, display_area):
        ...
        if sprites:
            if self.is_breakable:
                sprite_img = sprites.get("break_platform", None)
            else:
                sprite_img = sprites.get("std_platform", None)
        if sprite_img:
            sprite_img = pygame.transform.smoothscale(
                sprite_img, (self.rect.width, self.rect.height)
            )
            display_area.blit(sprite_img, (self.rect.x, self.rect.y))
        else:
            pygame.draw.rect(display_area, self.color, self.rect)
...
###################### 彈簧類別 ######################
class Spring:
    def __init__(self, x, y, width=35, height=20, color=(255, 255, 0)):
        """
        初始化彈簧道具
        x, y: 彈簧左上角座標 (通常在平台上方)
        width, height: 彈簧寬高 (預設35x20)
        color: 彈簧顏色 (預設黃色)
        """
    ...
    def draw(self, display_area):
        ...
        sprite_img = None
        if sprites:
            sprite_img = sprites.get("spring_normal", None)
        if sprite_img:
            sprite_img = pygame.transform.smoothscale(
                sprite_img, (self.rect.width, self.rect.height)
            )
...
###################### 主角設定 ######################
player_w = 50  # 主角寬度(步驟12: 50)
player_h = 50  # 主角高度(步驟12: 50)
...
###################### 平台設定 ######################
platform_w = 80  # 平台寬度(步驟12: 80)
platform_h = 20  # 平台高度(步驟12: 20)
...
for i in range(1, num_platforms):
    ...
    if random.random() < 0.2:
        spring_x = x + (platform_w - 35) // 2  # 彈簧水平置中於平台(步驟12: 35)
        spring_y = y - 20  # 彈簧放在平台上方(步驟12: 20)
        spring = Spring(spring_x, spring_y, 35, 20)
...
def update_camera(
    ...
    while len(platforms) < num_platforms + 10:
        ...
        if random.random() < 0.2:
            spring_x = new_x + (platform_w - 35) // 2
            spring_y = new_y - 20
            spring = Spring(spring_x, spring_y, 35, 20)
            springs.append(spring)
...
def load_doodle_sprites():
    """
    載入Doodle Jump所需的圖片精靈，並裁切src.png中的各物件圖片。
    回傳sprites字典，key為物件名稱，value為pygame.Surface物件。
    """
    # 使用sys.path[0]取得目前程式所在資料夾，縮短路徑
    base_path = sys.path[0]
    # 載入總表圖片src.png，並轉換為支援透明的格式
    src_path = os.path.join(base_path, "image", "src.png")
    if not os.path.exists(src_path):
        print(
            f"找不到圖片檔案: {src_path}\n請確認 image/src.png 是否存在於程式資料夾內。"
        )
        sys.exit(1)
    source_image = pygame.image.load(src_path).convert_alpha()
    # 定義各物件在src.png中的座標與寬高，及玩家圖片路徑
    sprite_data = {
        "std_platform": (3, 0, 113, 30),  # 標準平台
        "break_platform": (0, 145, 124, 33),  # 可破壞平台
        "spring_normal": (376, 188, 71, 35),  # 普通彈簧
        # 玩家圖片(直接路徑，不在src.png內)
        "player_left_jumping": os.path.join(base_path, "image", "l.png"),
        "player_left_falling": os.path.join(base_path, "image", "ls.png"),
        "player_right_jumping": os.path.join(base_path, "image", "r.png"),
        "player_right_falling": os.path.join(base_path, "image", "rs.png"),
    }
    sprites = {}  # 用來存放處理好的圖片
    for key, value in sprite_data.items():
        # 以 key 是否以 'player_' 開頭來判斷是否為玩家圖片
        if key.startswith("player_"):
            # 玩家圖片直接載入並轉換透明
            sprites[key] = pygame.image.load(value).convert_alpha()
        else:
            # 從src.png裁切出對應區域
            x, y, w, h = value
...
# 建立主角物件，並於載入sprites後再呼叫load_sprites
player = Player(player_x, player_y, player_w, player_h, player_color)
...
while True:
    ...
    screen.fill((255, 255, 255))  # 填滿白色背景(步驟12)
    # 呼叫主角的draw方法繪製主角
    player.draw(screen)
    # 繪製所有平台
    for platform in platforms:
