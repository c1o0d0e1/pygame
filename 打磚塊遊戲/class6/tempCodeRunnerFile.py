...
class Bomb:
    ...
    def get_center(self):
        ...
        if self.y_type == "bottom":
            ...
            y = (self.center_row) * (bricks_height + briaks_gap) + 60 + bricks_height // 2
        return x, y
    ...
    def draw(self, display_area, shake_offset=(0,0)):
        ...
        # 畫導火線
        pygame.draw.line(display_area, (180, 180, 180), (x, y - radius), (x, y - radius - 14), 4 if not self.is_super else 8)
        # 畫火花
        pygame.draw.circle(display_area, (255, 200, 0), (x, y - radius - 18), 6 if not self.is_super else 12)
                ...
                if 0 <= idx < len(bricks):
                    ...
                    if (
                        ...
                        if bomb.is_super:
                            ...
                            explosions.append({'x': bx, 'y': by, 'frame': 0, 'super': True})
                            shake_frames = 20
                            victory = True
                        else:
                            ...
                            bomb.visible = False
                            bx, by = bomb.get_center()
                            explosions.append({'x': bx, 'y': by, 'frame': 0, 'super': False})
                            shake_frames = 8
                        bomb_exploded = True
                        break
                if bomb_exploded:
